	...
	import com.amc.careplanner.web.rest.DisabilityTypeResource;
...
import com.amc.careplanner.service.DisabilityTypeQueryService;
	...
	*/
	...
	public class DisabilityTypeResourceExt extends DisabilityTypeResource{
	
	private final Logger log = LoggerFactory.getLogger(DisabilityTypeResourceExt.class);
	...
	private final DisabilityTypeServiceExt disabilityTypeServiceExt;
	...
	public DisabilityTypeResourceExt(DisabilityTypeServiceExt disabilityTypeServiceExt, DisabilityTypeQueryService disabilityTypeQueryService, UserRepositoryExt userRepositoryExt, SystemEventsHistoryServiceExt systemEventsHistoryServiceExt) {
	super(disabilityTypeServiceExt,disabilityTypeQueryService);
	this.disabilityTypeServiceExt = disabilityTypeServiceExt;
	...
	*/
	...
	DisabilityTypeDTO result = disabilityTypeServiceExt.save(disabilityTypeDTO);
	String loggedInAdminUserEmail = SecurityUtils.getCurrentUserLogin().get();
	User loggedInAdminUser = userRepositoryExt.findOneByEmailIgnoreCase(loggedInAdminUserEmail).get();  		
		CommonUtils.fireSystemEvent(systemEventsHistoryServiceExt, "createDisabilityType", "/api/v1/create-disability-type-by-client-id",
       result.getDisability() + " has just been created", "DisabilityType", result.getId(), loggedInAdminUser.getId(),
	...
	*/
	...
	String loggedInAdminUserEmail = SecurityUtils.getCurrentUserLogin().get();
	User loggedInAdminUser = userRepositoryExt.findOneByEmailIgnoreCase(loggedInAdminUserEmail).get();  		
		CommonUtils.fireSystemEvent(systemEventsHistoryServiceExt, "updateDisabilityType", "/api/v1/update-disability-type-by-client-id",
       result.getDisability() + " has just been updated", "DisabilityType", result.getId(), loggedInAdminUser.getId(),
    		loggedInAdminUser.getEmail(), result.getId());
