...
export const QuestionType = (props: IQuestionTypeProps) => {
  ...
  const { questionTypeList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="question-type-heading">
        <Translate contentKey="carePlannerApp.questionType.home.title">Question Types</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="carePlannerApp.questionType.home.createLabel">Create new Question Type</Translate>
        </Link>
      </h2>
        ...
        {questionTypeList && questionTypeList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('questionType')}>
                  <Translate contentKey="carePlannerApp.questionType.questionType">Question Type</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {questionType.lastUpdatedDate ? (
                      <TextFormat type="date" value={questionType.lastUpdatedDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${questionType.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${questionType.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="carePlannerApp.questionType.home.notFound">No Question Types found</Translate>
            </div>
          )
        )}
      </div>
...
const mapStateToProps = ({ questionType }: IRootState) => ({
  ...
  totalItems: questionType.totalItems,
});
