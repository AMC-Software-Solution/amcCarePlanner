...
export default class EligibilityUpdatePage {
  ...
  isEligibleInput: ElementFinder = element(by.css('input#eligibility-isEligible'));
  noteInput: ElementFinder = element(by.css('input#eligibility-note'));
  createdDateInput: ElementFinder = element(by.css('input#eligibility-createdDate'));
  ...
  hasExtraDataInput: ElementFinder = element(by.css('input#eligibility-hasExtraData'));
  ...
  async enterData() {
    ...
    const selectedIsEligible = await this.getIsEligibleInput().isSelected();
    if (selectedIsEligible) {
      await this.getIsEligibleInput().click();
      expect(await this.getIsEligibleInput().isSelected()).to.be.false;
    } else {
      await this.getIsEligibleInput().click();
      expect(await this.getIsEligibleInput().isSelected()).to.be.true;
    }
    await waitUntilDisplayed(this.saveButton);
