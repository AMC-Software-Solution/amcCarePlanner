...
export default class SystemEventsHistoryUpdatePage {
  ...
  callerEmailInput: ElementFinder = element(by.css('input#system-events-history-callerEmail'));
  callerIdInput: ElementFinder = element(by.css('input#system-events-history-callerId'));
  ...
  async setCallerEmailInput(callerEmail) {
    await this.callerEmailInput.sendKeys(callerEmail);
  }

  async getCallerEmailInput() {
    return this.callerEmailInput.getAttribute('value');
  }

  async setCallerIdInput(callerId) {
    await this.callerIdInput.sendKeys(callerId);
  }

  async getCallerIdInput() {
    return this.callerIdInput.getAttribute('value');
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setCallerEmailInput('callerEmail');
    expect(await this.getCallerEmailInput()).to.match(/callerEmail/);
    await waitUntilDisplayed(this.saveButton);
    await this.setCallerIdInput('5');
    expect(await this.getCallerIdInput()).to.eq('5');
    await waitUntilDisplayed(this.saveButton);
