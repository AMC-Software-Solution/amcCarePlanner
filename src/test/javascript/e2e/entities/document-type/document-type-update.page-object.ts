...
export default class DocumentTypeUpdatePage {
  ...
  documentTypeCodeInput: ElementFinder = element(by.css('input#document-type-documentTypeCode'));
  ...
  async setDocumentTypeCodeInput(documentTypeCode) {
    await this.documentTypeCodeInput.sendKeys(documentTypeCode);
  }

  async getDocumentTypeCodeInput() {
    return this.documentTypeCodeInput.getAttribute('value');
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setDocumentTypeCodeInput('documentTypeCode');
    expect(await this.getDocumentTypeCodeInput()).to.match(/documentTypeCode/);
    await waitUntilDisplayed(this.saveButton);
