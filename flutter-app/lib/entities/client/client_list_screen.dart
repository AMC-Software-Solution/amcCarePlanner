...
class ClientListScreen extends StatelessWidget {
    ClientListScreen({Key key}) : super(key: ClientRoutes.listScreenKey);
  ...
  Widget build(BuildContext context) {
    return  BlocListener<ClientBloc, ClientState>(
      listener: (context, state) {
        if(state.deleteStatus == ClientDeleteStatus.ok) {
          Navigator.of(context).pop();
          scaffoldKey.currentState.showSnackBar(new SnackBar(
              content: new Text(S.of(context).pageEntitiesClientDeleteOk)
          ));
        }
      },
      child: Scaffold(
          ...
          appBar: AppBar(
            centerTitle: true,
    title:Text(S.of(context).pageEntitiesClientListTitle),
          ),
              ...
              builder: (context, state) {
                return Visibility(
                  ...
                  child: Column(children: <Widget>[
                    for (Client client in state.clients) clientCard(client, context)
                  ]),
                );
              }
            ),
            ...
            ListTile(
              leading: Icon(
                ...
                color: Theme.of(context).primaryColor,
                  ...
                  title: Text('Client Name : ${client.clientName.toString()}'),
                  subtitle: Text('Client Description : ${client.clientDescription.toString()}'),
              trailing: DropdownButton(
                  ...
                  onChanged: (String newValue) {
                    switch (newValue) {
                      case "Edit":
                        Navigator.pushNamed(
                            context, ClientRoutes.edit,
                            arguments: EntityArguments(client.id));
                        break;
                      case "Delete":
                        showDialog(
                          context: context,
  ...
  Widget deleteDialog(ClientBloc clientBloc, BuildContext context, int id) {
    return BlocProvider<ClientBloc>.value(
      value: clientBloc,
      child: AlertDialog(
        title: new Text(S.of(context).pageEntitiesClientDeletePopupTitle),
        content: new Text(S.of(context).entityActionConfirmDelete),
        actions: <Widget>[
          new FlatButton(
            child: new Text(S.of(context).entityActionConfirmDeleteYes),
            onPressed: () {
              clientBloc.add(DeleteClientById(id: id));
            },
          ),
          new FlatButton(
